...
const CreatePost = () => {
    ...
    const handleSearch = async () => {
      const apiUrl = `/test/yelp/${location}?term=${searchQuery}`;
  ...
  const updatePostUrl = async (postId: string, newUrl: string) => {
    try {
      const response = await fetchAPI(`/test/post/${postId}`, {
        method: 'PUT',
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const response = await fetchAPI('/test/create-post', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        const postInfo = await fetchAPI('/test/user/erick/post'); // replace with current user
